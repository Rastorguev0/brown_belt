...
private:
	struct CfAccess {
		explicit CfAccess(Polynomial& p, size_t deg) : poly(p), degree(deg) {}
		const size_t degree;
		Polynomial& poly;

		T operator =(const T& value) {
				return poly.ChangeDegree(degree, value);
		}
	...
	T& ChangeDegree(size_t degree, T value) {
			if (degree > Degree()) coeffs_.resize(degree + 1);
			coeffs_[degree] = value;
			Shrink();
			T nul = 0;
			return degree > Degree() ? nul : coeffs_[degree];
	}
...
public:
	...
	Polynomial(size_t degree, size_t cf) {
		vector<T> poly(degree + 1, 0);
		poly[degree] = cf;
		coeffs_ = move(poly);
	}
	...
	CfAccess operator [](size_t degree) {
		return CfAccess(*this, degree);
	}
