...
struct Money {
	Money() = default;
	Money(double e, double s) : earn(e), spent(s) {}
	double earn = 0.0;
	double spent = 0.0;
...
Money operator+(const Money& lhs, const Money& rhs) {
	return { lhs.earn + rhs.earn, lhs.spent + rhs.spent };
}

array<double, VERTEX_COUNT> tree_factor, tree_spent;
array<Money, VERTEX_COUNT> tree_add, tree_values;

void Init() {
	//TODO ADD NEW FIELD FOR SPENT
	tree_values.fill(Money());
	tree_add.fill(Money());
	tree_factor.fill(1);
}
...
void Push(size_t v, size_t l, size_t r) {
	for (size_t w = v * 2; w <= v * 2 + 1; ++w) {
		if (w < VERTEX_COUNT) {
			...
			tree_add[w].earn *= tree_factor[v];
			tree_add[w].earn += tree_add[v].earn;
			tree_add[w].spent += tree_add[v].spent;
			tree_values[w].earn *= tree_factor[v];
			tree_values[w].earn += tree_add[v].earn * (r - l) / 2;
...
double ComputeSum(size_t v, size_t l, size_t r, size_t ql, size_t qr) {
	...
	Push(v, l, r);
	if (ql <= l && r <= qr) {
		return tree_values[v].Total();
	}
	return ComputeSum(v * 2, l, (l + r) / 2, ql, qr)
		+ ComputeSum(v * 2 + 1, (l + r) / 2, r, ql, qr);
}

void Add(size_t v, size_t l, size_t r, size_t ql, size_t qr, Money value) {
	if (v >= VERTEX_COUNT || qr <= l || r <= ql) {
		return;
	}
	Push(v, l, r);
	if (ql <= l && r <= qr) {
		tree_add[v].earn += value.earn;
		tree_add[v].spent += value.spent;
		tree_values[v].earn += value.earn * (r - l);
	...
	tree_values[v] =
		(v * 2 < VERTEX_COUNT ? tree_values[v * 2] : Money())
		+ (v * 2 + 1 < VERTEX_COUNT ? tree_values[v * 2 + 1] : Money());
}
...
ComputeIncome 2000-01-01 2001-01-01
	...
	for (int i = 0; i < q; ++i) {
		...
		if (query_type == "ComputeIncome") {
			...
			double value;
			INPUT >> value;
			Add(1, 0, DAY_COUNT_P2, idx_from, idx_to, Money(value / (idx_to - idx_from), 0.0));
		}
		else if (query_type == "Spend") {
			double value;
			INPUT >> value;
			Add(1, 0, DAY_COUNT_P2, idx_from, idx_to, Money(0, value / (idx_to - idx_from)));
		}
	}
