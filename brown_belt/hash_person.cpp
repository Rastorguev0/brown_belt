...
struct Person {
  ...
  bool operator==(const Person& other) const {
      return tie(other.name, other.height, other.weight, other.address)
...
struct AddressHasher {
    size_t operator() (const Address& a) const {
        size_t A = address_hasher(a.city);
        size_t B = address_hasher(a.street);
        size_t C = b_hasher(a.building);
        size_t x = 3571;
        return (A * x * x + B * x + C);
  }
    
    hash<string> address_hasher;
    hash<int> b_hasher;
};

struct PersonHasher {
    size_t operator() (const Person& p) const {
        size_t A = n_hash(p.name);
        size_t B = h_hash(p.height);
        size_t C = w_hash(p.weight);
        size_t D = a_hash(p.address);
        size_t x = 18181;
